 ...
 * @return 无
 */
that.createTheme = function(colour){
    localStorage.setItem('theme', colour);
    that.reloadFocuseTab();//刷新怎么来着
};
 ...
 * 调用Monaco里面的方法
 */
that.MonacoFun = function(funId){
    var tabId =  getFocusedTabId();
    var tabWebview = document.getElementById(tabId+'View');
    tabWebview.executeJavaScript('useMonacoFun("'+funId+'")', false);
}
/**
*打开P端模拟器
*/
that.creatPWindows = function(){
	const path = require('path');
	const url = require('url');
	let win = new browserWindow({ width: 1020, height: 720, show: false,resizable:false});
	win.on('closed', function() {
	  win = null;
	});
	win.loadURL(url.format({
    pathname: path.join(__dirname, '../p_port/index.html'),
    protocol: 'file:',
    slashes: true
  }));
	win.show();
